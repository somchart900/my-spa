...
// ฟังก์ชัน monitor loader 3 วิ
function monitorLoader() {
    setTimeout(() => {
        const contentEl = document.getElementById('app-content');
        if (!contentEl) return;

        // ตรวจว่ามี spinner หรือคำว่า Loading อยู่ไหม
        const spinner = contentEl.querySelector('.spinner-border');
        const loadingText = contentEl.textContent.toLowerCase().includes('loading');

        if (spinner || loadingText) {
            console.warn('Content stuck, redirect to home');
            window.app.router.navigate('404');
        }
    }, 500);
}

// เรียก monitor loader หลัง DOM พร้อม
